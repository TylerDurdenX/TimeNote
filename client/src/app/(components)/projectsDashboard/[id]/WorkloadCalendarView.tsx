...
import { useGetProjectTasksCalendarQuery, useGetProjectTasksQuery } from '@/store/api';
import { CalendarDays, Maximize2, Settings } from 'lucide-react';
...
const localizer = momentLocalizer(moment);
  ...
  const WorkloadCalendarView = ({ projectId, sprint, assignedTo, priority, isTaskOrSubTask, email }: Props) => {
    ...
    const {
        data,
      } = useGetProjectTasksCalendarQuery({ projectId, sprint, assignedTo, priority, isTaskOrSubTask, email: email! , page: 1, limit: 9999999999},{
            ...
            <div className="grid gap-4">
              ...
              <Maximize2 className="ml-3 cursor-pointer mt-0" />
                ...
                <p className="text-sm text-muted-foreground">
                  {event.description.split(' ').slice(0, 100).join(' ') + (event.description.split(' ').length > 100 ? '...' : '')}
                </p>
              </div>
              <div className="grid gap-2">
                ...
                <div className="grid grid-cols-5 items-center gap-4">
                  ...
                  <Label htmlFor="maxWidth" className='col-span-3 h-4 items-center justify-center mt-1'>{event.points}</Label>
                </div>
    ...
    return (
        <div style={{ height: '80vh', width: '100%' }} className='mt-5 ml-5 pr-6 w-screen'>
      <Calendar
        localizer={localizer}
        events={resultList}
