...
const UserWorkload = ({ projectId, sprint, assignedTo, priority, isTaskOrSubTask, email }: Props) => {
  ...
  return (
    <div className="flex gap-4 px-4 mr-4 h-full">
          ...
          <WorkloadCalendarView projectId={projectId} sprint= {sprint} email={email!} assignedTo={assignedTo} priority={priority} isTaskOrSubTask={isTaskOrSubTask} />
        )}
          </div>
        </div>
  )
