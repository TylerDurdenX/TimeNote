          ...
          worksheet.eachRow({ includeEmpty: true }, (row, rowIndex) => {
            ...
            row.eachCell({ includeEmpty: true }, (cell, colIndex) => {
              ...
              if (colIndex === 2 ) {
                if(!cell.text.trim()){
                  rowIsValid = false;
                  errors.push(`Row ${rowIndex} - Task Description cannot be empty.`);
                }else{
              ...
              if (colIndex === 4) {
                if (isNaN(Number(cell.text))) {
                  errors.push(`Row ${rowIndex} - Column ${colIndex} must be a number.`);
                }else{
                  if(!cell.text.trim()){
                    rowIsValid = false;
                    errors.push(`Row ${rowIndex} - Estimated hours cannot be empty.`);
                  }else{
                ...
                const sprintValue = cell.text?.trim();
                  if (!sprintValue) {
                    ...
                    if (!sprints.includes(cell.text.trim())) {
                      rowIsValid = false;
                      errors.push(`Row ${rowIndex} - Column ${colIndex} value is not correct. Found "${cell.text}".`);
                    }else{
