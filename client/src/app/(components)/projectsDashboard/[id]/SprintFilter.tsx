...
import { useGetProjectSprintQuery, useGetSprintQuery, useUpdateProjectSprintMutation } from "@/store/api";
...
export function SprintFilter({ sprint, setSprint, projectId }: Props) {
  ...
  const [updateSprint , {isLoading: isLoadingUpdateSprint}] = useUpdateProjectSprintMutation()
  ...
  const {data: sprintData,isLoading:loading, refetch} = useGetProjectSprintQuery({sprintId: sprintId},{refetchOnMountOrArgChange:true})
  ...
  React.useEffect(() => {
    ...
    if(sprintData){
      ...
      const formattedStartDate = new Date(sprintData?.startDate!).toISOString().split('T')[0];
      setStartDate(formattedStartDate)
      const formattedEndDate = new Date(sprintData?.endDate!).toISOString().split('T')[0];
                    ...
                    <DialogFooter>
                      <button
                        ...
                        className={`flex w-200px mt-7 justify-center bg-blue-600 rounded-md border border-transparent bg-blue-primary px-4 py-2 text-base font-medium text-white shadow-sm 
                          hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-600 focus-offset-2 ${!isFormValid() || isLoadingUpdateSprint
                          ? "cursor-not-allowed opacity-50"
                          : ""}`}
                        disabled={!isFormValid() || isLoadingUpdateSprint}
                      >
                        {isLoadingUpdateSprint ? "Updating..." : "Update Sprint"}
                      </button>
                    </DialogFooter>
                  </form>
                </DialogContent>
