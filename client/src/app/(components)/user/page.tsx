...
const page = () => {
   ...
   const {data: dataUser , isLoading, error} = useGetUserDataQuery({username: username!},
        ...
        <div className="flex w-full space-x-6">
          ...
          <div className="w-1/4 flex justify-center items-center">
            <Avatar className="h-24 w-24 rounded-full shadow-lg">
              <AvatarImage src={dataUser?.profilePicture?.base64} alt={dataUser?.username} />
  ...
  <div className="flex flex-col space-y-2 w-1/2">
    ...
    <ul className="list-disc pl-6 space-y-1 text-gray-700">
      {dataUser?.projects?.map((project, index) => (
        <li key={index} className="text-sm">{project.name}</li>
      ))}
    </ul>
  </div>
  ...
  <div className="flex flex-col space-y-2 w-1/2">
    ...
    <ul className="list-disc pl-6 space-y-1 text-gray-700">
      {dataUser?.teams?.map((team, index) => (
        <li key={index} className="text-sm">{team.name}</li>
      ))}
    </ul>
  </div>
</div>
