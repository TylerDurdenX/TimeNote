...
import { useGetUserAttendanceTeamReportDataQuery } from "@/store/api";
...
const AttendanceReportTable = ({
  ...
  const columns = [
    ...
    { field: "presentDays", headerName: "Present Days", width: 200 },
  ];
  ...
  const columnGroupingModel = useMemo(() => {
    return Array.from({ length: daysInMonth }, (_, day) => ({
      groupId: `${day + 1}/${month + 1}/${currentYear}`,
      children: [
  ...
  const { data, isLoading, error } = useGetUserAttendanceTeamReportDataQuery(
    {
      teamName,
      month,
