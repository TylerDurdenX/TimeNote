...
import {  useDeleteBreakMutation, useGetBreaksQuery, useGetTeamsQuery, useUpdateBreakDetailsMutation } from '@/store/api';
...
const BreaksTable = () => {
  ...
  const {data: breakData, isLoading} = useGetBreaksQuery({email: userEmail!},
    ...
    const handleBreakNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const handleBreakDescriptionChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
        // Get the current cursor position
        const cursorPosition = e.target.selectionStart;
        const value = e.target.value;
        ...
        setTimeout(() => {
          if (breakDescriptionRef.current) {
            breakDescriptionRef.current.setSelectionRange(cursorPosition, cursorPosition);
          }
        }, 0); // Ensures cursor position restoration happens after the state change
      };
    
      // Handle input change for break time with cursor position fix
      const handleBreakTimeInMinutesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        // Get the current cursor position
        const cursorPosition = e.target.selectionStart;
        const value = e.target.value;
        ...
        setTimeout(() => {
          if (breakTimeInMinutesRef.current) {
            breakTimeInMinutesRef.current.setSelectionRange(cursorPosition, cursorPosition);
          }
        }, 0); // Ensures cursor position restoration happens after the state change
      };
  ...
  const handleUpdateBreakDetails = async() => {
      try {
          const response = await updateBreakDetails({email: userEmail!,breakId: id, breakName: breakName, breakDescription: breakDescription,
              ...
              <Dialog open={isOpen} onOpenChange={setIsOpen}>
                ...
                <div className="grid gap-4 py-9">
                  <div className="grid grid-cols-8 items-center gap-4">
                    <Label className="text-center">Break Name<span className="text-red-500 ml-1">*</span></Label>
                        <Input
                         ...
                         required
                       />
                        <Label className="text-center">Break Description<span className="text-red-500 ml-1">*</span></Label>
                        <textarea
                         ...
                         required
                       />
                       <Label className="text-center">Break Time<span className="text-red-500 ml-1">*</span></Label>
                        <Input
