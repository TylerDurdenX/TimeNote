...
import { formatDate, formatTime } from "../attendanceController/attendanceController.js";
...
export const createBreak = catchAsync(async (req, res, next) => {
  ...
  export const updateBreakTime = catchAsync(async (req, res, next) => {
    ...
    try {
      await prisma.$transaction(async (prisma) => {
        ...
        if(breakTypeCode !== 'CUSTOM_BREAK' ){
          ...
          const breakTaken = await prisma.breaks.update({
            ...
            data:{
              endTime: endTime,
              breakTimeInMinutes: `${totalMinutes}:${remainingSeconds.toString().padStart(2, '0')}`
            }
          })
          resultBreakId = breakTaken.id
        }
      ...
      await Promise.all(inProgressTasks.map(async (task) => {
        const differenceInMilliseconds = currentDateTime - new Date(task.inProgressStartTime);
...
function subtractTime(minutesStr, timeStr) {
  ...
  if (diffInSeconds < 0) {
      ...
      return `${String(exceededMinutes).padStart(2, '0')}:${String(exceededSeconds).padStart(2, '0')}`;
  } else {
      return "0";
  }
}
  ...
  export const getBreaksData = catchAsync(async (req, res, next) => {
    ...
    try {
      ...
      await Promise.all(breaksList.map( async (breakTaken) => {
        ...
        const breakObj = {
          ...
          breakTimeOverrun: subtractTime(breakTimeConfigured, breakTaken.breakTimeInMinutes)
        }
        id = id+1
